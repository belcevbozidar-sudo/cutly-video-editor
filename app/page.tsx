...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace"><header className="topbar"><div className="breadcrumb"><span>Моите проекти</span><span className="slash">/</span><input value={projectName} onChange={(e) => setProjectName(e.target.value)} aria-label="Име на проекта" /></div><div className="top-actions"><button className="text-button" onClick={saveProject}>{saved ? <><Check size={16} /> Запазено</> : "Запази"}</button><button className="export-button" disabled={!video || exporting} onClick={exportVideo}><Download size={17} /> {exporting ? "Изнасяне…" : "Изнеси MP4"}</button></div></header>
      ...
      <section className="timeline-panel"><div className="timeline-tools"><button className="timeline-tool active"><Scissors size={16} /> Изрязване</button><button className="timeline-tool" onClick={() => importMoreVideoRef.current?.click()}><Plus size={16} /> Добави клип</button><button className="timeline-tool" onClick={() => importMusicRef.current?.click()}><Music2 size={16} /> Музика</button><button className="timeline-tool" onClick={() => setCaptionsOn(true)}><Subtitles size={16} /> Субтитри</button><span className="timeline-hint">{exportStatus || "Кликни върху линията, за да преместиш маркера"}</span></div>{clips.length > 0 && <div className="scene-strip">{clips.map((clip, index) => <button key={clip.url} className={`scene-chip ${video?.url === clip.url ? "active" : ""}`} onClick={() => selectClip(clip)}><span>{index + 1}</span><FileVideo size={13} />{clip.name}</button>)}</div>}<div className="ruler">{[0, .2, .4, .6, .8, 1].map((tick) => <span key={tick} style={{ left: `${tick * 100}%` }}>{timeLabel((duration || 25) * tick)}</span>)}</div><div className="track-area" onClick={(event) => { if (!video || !duration) return; const bounds = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - bounds.left) / bounds.width) * duration); }}><div className="track-video"><FileVideo size={18} /><span>{video?.name || "Качи видео, за да започнеш"}</span></div>{video && <><div className="selection" style={{ left: `${(start / total) * 100}%`, width: `${selectionWidth}%` }}><i className="trim-handle left" /><i className="trim-handle right" /></div>{middleCutOn && middleCutEnd > middleCutStart && <div className="middle-cut" style={{ left: `${(middleCutStart / total) * 100}%`, width: `${((middleCutEnd - middleCutStart) / total) * 100}%` }} />}<div className="playhead" style={{ left: `${(current / total) * 100}%` }} /></>}</div><div className="audio-track"><Music2 size={16} /><span>{music?.name || "Добави фонова музика"}</span></div></section></section>
