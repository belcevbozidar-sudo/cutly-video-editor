...
import { ArrowLeft, ArrowRight, AudioLines, Check, ChevronDown, CircleHelp, Clapperboard, Download, FileVideo, FolderOpen, Gauge, Maximize2, Music2, Pause, Play, Plus, Scissors, SlidersHorizontal, Sparkles, Subtitles, Type, Upload, Volume2, Wand2, X } from "lucide-react";
...
export default function Home() {
  ...
  const importMoreVideoRef = useRef<HTMLInputElement>(null);
  ...
  function importAsset(event: ChangeEvent<HTMLInputElement>, kind: "video" | "music" | "voice") {
    ...
    if (kind === "video") { setVideo(asset); setClips([asset]); setStart(0); setEnd(0); setCurrent(0); setIsPlaying(false); }
    if (kind === "music") setMusic(asset); if (kind === "voice") setVoice(asset); event.target.value = "";
  }
  function addClips(event: ChangeEvent<HTMLInputElement>) {
    const files = Array.from(event.target.files || []); if (!files.length) return;
    const assets = files.map((file) => ({ name: file.name.replace(/\.[^/.]+$/, ""), url: URL.createObjectURL(file), file }));
    setClips((existing) => [...existing, ...assets]);
    if (!video) setVideo(assets[0]);
    event.target.value = "";
  }
  function selectClip(clip: MediaAsset) {
    setVideo(clip); setStart(0); setEnd(0); setCurrent(0); setMiddleCutOn(false); setIsPlaying(false);
  }
  function moveClip(index: number, direction: -1 | 1) {
    const target = index + direction;
    if (target < 0 || target >= clips.length) return;
    setClips((items) => { const next = [...items]; [next[index], next[target]] = [next[target], next[index]]; return next; });
  }
  function removeClip(index: number) {
    const removed = clips[index]; const next = clips.filter((_, itemIndex) => itemIndex !== index);
    setClips(next); if (video?.url === removed.url) { const replacement = next[Math.max(0, index - 1)]; if (replacement) selectClip(replacement); else { setVideo(null); setDuration(0); setStart(0); setEnd(0); } }
  }
  ...
  async function exportVideo() {
    ...
    try {
      ...
      const renderClips = clips.length ? clips : [video];
      for (const [index, clip] of renderClips.entries()) { if (!clip.file) throw new Error("Липсва файл на сцена"); await ffmpeg.writeFile(`clip${index}`, await fetchFile(clip.file)); }
      ...
      const sourceFilter = renderClips.length > 1
        ? `${renderClips.map((_, index) => `[${index}:v]setpts=PTS-STARTPTS[v${index}];[${index}:a]asetpts=PTS-STARTPTS[a${index}]`).join(";")};${renderClips.map((_, index) => `[v${index}][a${index}]`).join("")}concat=n=${renderClips.length}:v=1:a=1[basev][basea]`
        : middleCutOn && middleCutEnd > middleCutStart
      ...
      const inputs = renderClips.flatMap((_, index) => ["-i", `clip${index}`]);
      const audioMix = ["[basea]atempo=" + speed + ",volume=1[a0]"];
      const mixLabels = ["[a0]"];
      if (music?.file) { const musicIndex = renderClips.length; inputs.push("-stream_loop", "-1", "-i", "music"); audioMix.push(`[${musicIndex}:a]volume=${(musicVolume / 100).toFixed(2)}[a1]`); mixLabels.push("[a1]"); }
      if (voice?.file) { const voiceIndex = renderClips.length + (music?.file ? 1 : 0); inputs.push("-i", "voice"); audioMix.push(`[${voiceIndex}:a]volume=${(voiceVolume / 100).toFixed(2)}[a2]`); mixLabels.push("[a2]"); }
      ...
      setExportStatus(`Готово — ${renderClips.length > 1 ? `${renderClips.length} сцени са обединени в ` : ""}MP4 файлът се сваля.`);
  ...
  return <main className="app-shell">
    ...
    <input ref={importMoreVideoRef} className="sr-only" accept="video/*" multiple type="file" onChange={addClips} />
    <audio ref={musicRef} src={music?.url} loop /><audio ref={voiceRef} src={voice?.url} />
    <aside className="sidebar"><div className="brand"><span className="brand-mark"><Clapperboard size={20} /></span><span>cutly</span></div><button className="new-project" onClick={() => importVideoRef.current?.click()}><Plus size={18} /> Нов проект</button><button className="add-scene-side" onClick={() => importMoreVideoRef.current?.click()}><Plus size={16} /> Добави сцени</button><div className="nav-label">РАБОТНО ПРОСТРАНСТВО</div><button className="nav-item active"><FolderOpen size={18} /> Моите проекти <span>{Math.max(1, clips.length)}</span></button><button className="nav-item" onClick={() => setHelpOpen(true)}><Sparkles size={18} /> AI инструменти</button><div className="sidebar-bottom"><button className="help-link" onClick={() => setHelpOpen(true)}><CircleHelp size={17} /> Как работи?</button><div className="profile"><div>Б</div><span>Bozhidar</span><ChevronDown size={15} /></div></div></aside>
    <section className="workspace"><header className="topbar"><div className="breadcrumb"><span>Моите проекти</span><span className="slash">/</span><input value={projectName} onChange={(e) => setProjectName(e.target.value)} aria-label="Име на проекта" /></div><div className="top-actions"><button className="text-button" onClick={saveProject}>{saved ? <><Check size={16} /> Запазено</> : "Запази"}</button><button className="export-button" disabled={!video || exporting} onClick={exportVideo}><Download size={17} /> {exporting ? "Изнасяне…" : "Изнеси MP4"}</button></div></header>
      ...
      <section className="timeline-panel"><div className="timeline-tools"><button className="timeline-tool active"><Scissors size={16} /> Изрязване</button><button className="timeline-tool" onClick={() => importMoreVideoRef.current?.click()}><Plus size={16} /> Добави клип</button><button className="timeline-tool" onClick={() => importMusicRef.current?.click()}><Music2 size={16} /> Музика</button><button className="timeline-tool" onClick={() => setCaptionsOn(true)}><Subtitles size={16} /> Субтитри</button><span className="timeline-hint">{exportStatus || "Кликни върху линията, за да преместиш маркера"}</span></div>{clips.length > 0 && <div className="scene-strip">{clips.map((clip, index) => <div key={clip.url} className={`scene-chip ${video?.url === clip.url ? "active" : ""}`}><button className="scene-main" onClick={() => selectClip(clip)}><span>{index + 1}</span><FileVideo size={13} />{clip.name}</button><button aria-label="Премести наляво" disabled={index === 0} onClick={() => moveClip(index, -1)}><ArrowLeft size={12} /></button><button aria-label="Премести надясно" disabled={index === clips.length - 1} onClick={() => moveClip(index, 1)}><ArrowRight size={12} /></button><button aria-label="Премахни сцена" onClick={() => removeClip(index)}><X size={12} /></button></div>)}</div>}<div className="ruler">{[0, .2, .4, .6, .8, 1].map((tick) => <span key={tick} style={{ left: `${tick * 100}%` }}>{timeLabel((duration || 25) * tick)}</span>)}</div><div className="track-area" onClick={(event) => { if (!video || !duration) return; const bounds = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - bounds.left) / bounds.width) * duration); }}><div className="track-video"><FileVideo size={18} /><span>{video?.name || "Качи видео, за да започнеш"}</span></div>{video && <><div className="selection" style={{ left: `${(start / total) * 100}%`, width: `${selectionWidth}%` }}><i className="trim-handle left" /><i className="trim-handle right" /></div>{middleCutOn && middleCutEnd > middleCutStart && <div className="middle-cut" style={{ left: `${(middleCutStart / total) * 100}%`, width: `${((middleCutEnd - middleCutStart) / total) * 100}%` }} />}<div className="playhead" style={{ left: `${(current / total) * 100}%` }} /></>}</div><div className="audio-track"><Music2 size={16} /><span>{music?.name || "Добави фонова музика"}</span></div></section></section>
